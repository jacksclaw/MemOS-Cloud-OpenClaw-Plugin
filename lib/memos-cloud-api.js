...
export function buildConfig(pluginConfig = {}) {
  ...
  // JAC-246: "chat-type" scopes group chats to conversation_id, allows DMs global recall
  const recallScope = cfg.recallScope || loadEnvVar("MEMOS_RECALL_SCOPE") || "";
  ...
  // Auto-detect local MemOS server. Local server (localhost/127.0.0.1) uses different
  // API paths than the cloud API. No config required — detection is based on baseUrl.
  // Cloud: /search/memory + /add/message (requires apiKey)
  // Local: /product/search + /product/add  (no apiKey needed)
  const isLocalServer = /localhost|127\.0\.0\.1/.test(baseUrl);
  const searchPath = isLocalServer ? "/product/search" : "/search/memory";
  const addPath = isLocalServer ? "/product/add" : "/add/message";
...
export async function callApi({ baseUrl, apiKey, timeoutMs = 5000, retries = 1 }, path, body) {
  // apiKey is required for cloud (Token auth); optional for local server (no auth needed)
...
/**
 * Normalize a local MemOS /product/search response to the cloud API format.
 * Cloud format: { memory_detail_list, preference_detail_list, tool_memory_detail_list }
 * Local format: { text_mem: [{cube_id, memories: [{id, memory, metadata}]}], pref_mem: [...] }
 */
function normalizeLocalSearchResponse(result) {
  if (!result || result.code === undefined) return result; // already cloud format or null
  if (result.code !== 200) return result;

  const data = result.data || {};
  const memory_detail_list = [];
  const preference_detail_list = [];
  const tool_memory_detail_list = [];

  for (const group of data.text_mem || []) {
    for (const mem of group.memories || []) {
      memory_detail_list.push({
        memory_id: mem.id,
        memory_key: mem.memory,
  ...
  for (const group of data.pref_mem || []) {
    for (const mem of group.memories || []) {
      preference_detail_list.push({
        memory_id: mem.id,
        preference: mem.memory,
        create_time: mem.metadata?.created_at || "",
      });
    }
  }

  for (const group of data.tool_mem || []) {
    for (const mem of group.memories || []) {
      tool_memory_detail_list.push({
        memory_id: mem.id,
  ...
  return {
    // Wrap in structure that extractResultData(result) can unwrap via result.data
    data: {
      memory_detail_list,
      preference_detail_list,
      preference_note: data.pref_note || "",
...
export async function searchMemory(cfg, payload) {
  const result = await callApi(cfg, cfg.searchPath || "/search/memory", payload);
  // Normalize local MemOS response to cloud format so prompt builders work correctly
  return normalizeLocalSearchResponse(result);
}

/**
 * Normalize a cloud-format add payload for local MemOS /product/add.
 * Key differences:
 *   - async_mode: cloud sends boolean (true/false); local expects Literal["async","sync"]
 *   - tags: cloud uses "tags"; local expects "custom_tags"
 * Extra cloud-only fields (conversation_id, source, allow_public, allow_knowledgebase_ids)
 * are harmlessly ignored by Pydantic's default extra="ignore" policy.
 */
function normalizeLocalAddPayload(payload) {
  const out = { ...payload };

  // async_mode: boolean → "async" | "sync"
  if (typeof out.async_mode === "boolean") {
    out.async_mode = out.async_mode ? "async" : "sync";
  }

  // tags → custom_tags (local field name)
  if (out.tags !== undefined && out.custom_tags === undefined) {
    out.custom_tags = out.tags;
    delete out.tags;
  }
...
export async function addMessage(cfg, payload) {
  const path = cfg.addPath || "/add/message";
  // Normalize payload for local MemOS server (different schema from cloud API)
  const body = path === "/product/add" ? normalizeLocalAddPayload(payload) : payload;
  return callApi(cfg, path, body);
}
