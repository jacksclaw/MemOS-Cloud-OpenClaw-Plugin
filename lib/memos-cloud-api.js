...
export function buildConfig(pluginConfig = {}) {
  ...
  // Path overrides — use these to point at a local MemOS server whose API paths differ
  // from the cloud API (local: /product/search, /product/add vs cloud: /search/memory, /add/message)
  const searchPath = cfg.searchPath || loadEnvVar("MEMOS_SEARCH_PATH") || "/search/memory";
  const addPath = cfg.addPath || loadEnvVar("MEMOS_ADD_PATH") || "/add/message";
